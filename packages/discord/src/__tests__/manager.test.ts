...
describe("DiscordManager handleMessage pipeline", () => {
  ...
  it("keeps a bot/schedule post from allMessages in the since-block even when messages capped it out", async () => {
    const triggerCalls: unknown[][] = [];
    const { manager, connector } = buildManagerWithTrigger(async (...args: unknown[]) => {
      triggerCalls.push(args);
      return {
    ...
    const botPost = {
      ...
      isBot: true,
      isSelf: true,
      content: "schedule job proposal, capped out of the prioritized window",
      timestamp: "2024-01-01T00:06:00.000Z",
      messageId: "m-bot",
    };
    const humanMessages = Array.from({ length: 12 }, (_, i) => ({
      authorId: "user2",
      authorName: "OtherUser",
      isBot: false,
      isSelf: false,
      content: `human message ${i + 1}`,
      timestamp: `2024-01-01T00:${String(7 + i).padStart(2, "0")}:00.000Z`,
      messageId: `m-human-${i + 1}`,
    }));

    // Simulates buildConversationContext's prioritizeUserMessages cap: with 12
    // human messages > maxMessages (10), the older bot post never makes it
    // into `messages` even though it's newer than lastMessageAt.
    event.context.messages = humanMessages.slice(0, 10);
    // `allMessages` is the full fetched+filtered window (fetchLimit=20), which
    // still has the bot post — this is what the since-filter must read.
    event.context.allMessages = [botPost, ...humanMessages];
    ...
    const options = triggerCalls[0][2] as { prompt: string };
    expect(options.prompt).toContain("Messages posted in this channel since your last turn:");
    expect(options.prompt).toContain("schedule job proposal, capped out of the prioritized window");
  });
